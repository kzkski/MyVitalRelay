...
- MFA 無効 → refresh token 自動更新で **無人 Webhook 運用可**（INSERT → pg_net → GitHub Actions）
...
Claude ──INSERT (claude)──────┤
                    ...
                    pg_net → Edge Function → GitHub Actions
                              │
                    Garmin Sync Job → garmin_activity_archive / garmin_daily_archive
                              │
Claude ──SELECT garmin_*_claude──┘
```
...
| `garmin_daily_archive` | 日次 API JSON |
...
- `scope=activities` / `daily` / `all` 対応
- トリガー: pg_net → Edge Function → `repository_dispatch`（cron は廃止）
...
| Phase | 内容 | 状態 |
|---|---|---|
| **0** | `garmin_export_samples.py` ローカル実行 | ✅ |
| **1** | DDL + Sync Job + GHA | ✅ |
| **1.5** | pg_net Webhook 即時 trigger + 日次 sync | ✅ |
| **2** | 初回バックフィル（90 日） | ✅ |
| **3** | Claude ガイド整備 | ✅ |
...
- [ ] **認証情報の置き場所** — GitHub Actions Secrets（Sync Job）+ Supabase Secrets（Webhook dispatch）で確定
