...
| 利用可 | 未対応 / 後回し |
|---|---|
| ワークアウト詳細（FIT + API） | Supabase Webhook 即時トリガー（後回し） |
| View: `garmin_activity_claude_summary` | |
| View: `garmin_activity_claude` | |
| `garmin_sync_request` の `scope=activities` | |
...
## 5. 日次データ（Training Readiness / HRV 等）
...
```http
GET /rest/v1/garmin_daily_claude?date=eq.2026-07-12&select=training_readiness,hrv,body_battery,sleep
```

**まだ sync していない日は 0 件で正常。** 上記 POST → Sync Job 実行 → 再 SELECT。

ユーザーが回復状態を聞いてきた場合:

- 先に `garmin_daily_claude` を確認
...
| **1.5** | 日次 sync（実装済み）+ Webhook 即時トリガー（**後回し**） |
| **2** | 90 日バックフィル workflow（実装済み） |
| **3** | 本ガイド整備・`complete` 判定改善（実装済み） |
