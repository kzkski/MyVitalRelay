...
| 日次 Garmin | `scope=daily` / `garmin_daily_summary`（軽量）・`garmin_daily_claude`（深掘り） |
...
| 日次・回復（日常チェック） | `garmin_daily_summary` | `garmin_daily_archive`（内部テーブル） |
| 日次・回復（深掘り） | `garmin_daily_claude` | `garmin_daily_archive`（内部テーブル） |
...
| トレーニングステータス | `training_phrase`, `acwr`, `vo2max` | 負荷状態・VO2 Max |
| リカバリータイム | ワークアウト詳細 or 日次 JSON 内（深掘り時） | 前回練習からの回復残時間（機種・記録による） |

5 分おき HRV 配列・睡眠ステージ詳細・`api_responses` 全量が必要なときだけ `garmin_daily_claude` を使う。
...
- 先に `garmin_daily_summary` を確認（**`garmin_daily_claude` は深掘り時のみ**。1 行 ~573KB のため日常参照に使わない）
- `training_readiness` が `[]` でも **異常ではない**（255 では常に空。深掘り時のみ `garmin_daily_claude` で確認）
...
| `garmin_daily_summary` が空 = 失敗 | **未 sync の日は空で正常**。`scope=daily` で依頼 |
| 日次は `garmin_daily_claude` を読む | 日常は **`garmin_daily_summary`**（数百バイト/行）。深掘り時のみ `garmin_daily_claude` |
...
`hrv`, `body_battery`, `sleep`, `api_responses`（大型 JSONB）を含む。
...
| **3.1** | 日次軽量 VIEW `garmin_daily_summary`（Issue #22） | ✅ |
