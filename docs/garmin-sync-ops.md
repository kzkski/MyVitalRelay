...
7. `apply_garmin_daily_calories_to_summary(user_id, date_from, date_to)`（daily 含む場合。Issue #29。`get_stats` → `daily_activity_summary`。活動日 D → 格納日 D+1。冪等。skip 済み日も反映）
8. `progress` JSON を記録し、`complete` / `partial` / `failed` に更新
