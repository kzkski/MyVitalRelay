...
- `rpe` / `condition_notes` / 手動記録（`data_source='manual'`）: チャット上の Claude が記入する運用で、本アプリの対象外。Garmin 行への追記は **`upsert_training_log_annotation` RPC**（または注釈列のみ PATCH）。手順は `docs/claude-garmin-access.md` §4.5（Issue #33）。論理キー欠落の garmin INSERT は禁止（DB CHECK あり）。
