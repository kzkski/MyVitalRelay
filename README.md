...
- `rpe` / `condition_notes` / 手動記録（`data_source='manual'`）: チャット上の Claude が記入する運用で、本アプリの対象外。Garmin 行への追記は **既存行への PATCH（注釈列のみ）** または将来の `upsert_training_log_annotation` RPC（`docs/claude-garmin-access.md` §4.5 / Issue #33）。論理キー欠落の garmin INSERT は禁止。
